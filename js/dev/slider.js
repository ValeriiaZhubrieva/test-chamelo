...
window.initSliders = function() {
	...
	if (document.querySelector(".presentation__slider")) {
		const sliderElement = document.querySelector(".presentation__slider");
		const parent = sliderElement.closest(".presentation");
		const pagination = parent.querySelector(".presentation__pagination");
		const section = document.querySelector("[data-fixed-scroll]");
		const header = document.querySelector("header");
		const bgItems = parent.querySelectorAll(".presentation__bg-item");
		const swiper = new Swiper(sliderElement, {
			...
			pagination: {
				...
				renderBullet: function(index, className) {
					return `<span class="${className}">${document.querySelector(`.presentation__slide:nth-child(${index + 1}) .presentation__slide-title`).innerHTML}</span>`;
				}
			}
		});
		const slidesCount = swiper.slides.length;
		const maxIndex = slidesCount - 1;
		const segments = slidesCount;
		...
		function setSectionHeight() {
			const vh = getVH();
			section.style.height = `${segments * vh}px`;
		}
		function setSectionSlideClass(index) {
			section.className = section.className.replace(/\bactive-slide-\d+\b/g, "").trim();
			section.classList.add(`active-slide-${index + 1}`);
		}
		...
		function setSlideVars(index) {
			const activeSlide = swiper.slides[index];
			if (!activeSlide) return;
			const w = activeSlide.offsetWidth;
			const h = activeSlide.offsetHeight;
			section.style.setProperty("--slide-width", `${w}px`);
			section.style.setProperty("--slide-height", `${h}px`);
		}
		let lastIndex = -1;
		function updateSwiperByScroll() {
			const rect = section.getBoundingClientRect();
			const scrollable = section.offsetHeight - getVH();
			let progress = -rect.top / scrollable;
			progress = Math.min(Math.max(progress, 0), 1);
			const rawIndex = progress * maxIndex;
			const newIndex = Math.floor(rawIndex + .5);
			if (newIndex !== lastIndex) {
				lastIndex = newIndex;
				setSectionSlideClass(newIndex);
				updateBackground(newIndex);
				swiper.slideTo(newIndex);
				setSlideVars(newIndex);
			}
		}
		setSectionHeight();
		updateBackground(0);
		setSlideVars(0);
		updateSwiperByScroll();
		window.addEventListener("scroll", updateSwiperByScroll, { passive: true });
		window.addEventListener("resize", () => {
			updateSwiperByScroll();
		});
	}
	if (document.querySelector(".product-detail__slider")) new Swiper(".product-detail__slider", {
	...
	if (productFeatures.length) productFeatures.forEach((block) => {
		const blockImageSlider = block.querySelector(".product-features__slider");
		const blockDescrSlider = block.querySelector(".product-features__descr");
		const paginationEl = block.querySelector(".product-features__pagination");
		...
		const swiperText = new Swiper(blockDescrSlider, {
			...
			pagination: {
				...
				clickable: true,
				renderBullet: function(index, className) {
					const slideTitleEl = block.querySelectorAll(".product-features__item-title")[index];
					return `<span class="${className}">${slideTitleEl ? slideTitleEl.innerHTML : index + 1}</span>`;
				}
			}
		});
		swiperImg.controller.control = swiperText;
		swiperText.controller.control = swiperImg;
		paginationEl.addEventListener("mouseover", (e) => {
			if (e.target.classList.contains("swiper-pagination-bullet")) {
				const index = Array.from(paginationEl.children).indexOf(e.target);
				swiperText.slideTo(index);
			}
		});
	});
	if (document.querySelector(".our-products__slider")) document.querySelectorAll(".our-products__slider").forEach((el) => {
